...
| Cek ejaan | Typo lolos saat buru-buru | Kamus/regex bahasa Indonesia, bukan AI |
...
| Model AI | **Tidak dipakai sama sekali di Fase 1** | Semua pemeriksaan di bagian 3 berjawaban pasti. Fase 2 nanti memakai **Azure OpenAI** (deployment `gpt-4.1`), bukan ChatGPT konsumen |
| Pencarian peraturan | OpenSearch | **Tidak dipakai di Fase 1.** Baru relevan di Fase 3 |
| Database | **Tidak dipakai di Fase 1** | Lihat bagian 9, beserta alasannya |

### Syarat teknis add-in Word

Tidak ada hubungannya dengan env. Yang dibutuhkan tiga hal:
...
3. **Cara pemasangan** — dua jalur berbeda:

| Jalur | Untuk apa | Butuh izin admin? |
|---|---|---|
| Sideload dari folder share | Prototipe, pengujian sendiri | **Tidak.** Daftarkan foldernya di Word > Options > Trust Center > Trusted Add-in Catalogs. Windows saja, resmi bukan untuk produksi |
| Centralized Deployment (Microsoft 365 admin center) | Dipakai banyak penelaah | **Ya** — izin IT Kemenkeu |

**Tanyakan ke tim Law Analyzer duluan.** Law Analyzer sudah punya panel di Word
(brief bagian 8.5), artinya jalur deployment ini sudah pernah ditempuh di
Kemenkeu. Menanyakan manifest mereka dipasang lewat jalur apa dan siapa yang
menyetujui bisa menghemat berminggu-minggu.
...
backend/app/models/temuan.py                     [BARU]
  ===> skema Pydantic bentuk Temuan (bagian 11)
       Tetapkan ini DULU sebelum rules/ ditulis

frontend/src/app/taskpane/page.tsx
  ===> UI task pane: tombol Analisis, daftar temuan ringkas, detail kutipan
       fetch("http://localhost:8000/analisis/jalankan") — URL lengkap,
       bukan path relatif
...
- Temuan hidup di task pane selama dokumen terbuka.
...
### Soal Neon — ada yang perlu diputuskan dulu

Neon itu Postgres serverless yang di-host **di luar infrastruktur Kemenkeu**,
region mana pun yang dipilih. Untuk pengembangan dengan dokumen contoh, tidak
masalah dan memang praktis.

Untuk rancangan sungguhan, dua hal di dokumen proyek sendiri bertabrakan dengan
itu:

- Brief bagian 2 poin 5: *"rancangan yang belum terbit tidak layak dikirim ke
  layanan luar."*
- Brief bagian 8.10: kerahasiaan rancangan ditangani dengan *"kredensial hanya
  di sisi server; pengembangan memakai dokumen contoh."*

Rancangan PMK yang belum terbit adalah dokumen yang belum boleh keluar. Skema di
atas memang tidak menyimpan teks pasal utuh, tapi `catatan` dan
`usulan_rumusan` hampir pasti mengutip isi rancangan.

Pertimbangan kedua: tujuan akhir proyek ini **dimigrasikan jadi menu di dalam
Law Analyzer**. Law Analyzer memakai PostgreSQL di infrastruktur sendiri
(`POSTGRES_HOST=localhost` di berkas env-nya). Memakai Postgres yang sama
membuat migrasi nyaris tanpa pekerjaan tambahan; memakai Neon berarti tetap
harus migrasi nanti.

**Rekomendasi: Postgres internal untuk data sungguhan, Neon boleh untuk
pengembangan dengan dokumen contoh.** Keduanya Postgres, jadi kodenya sama
persis — cuma `DATABASE_URL` yang berbeda. Tidak ada yang terbuang.

Kalau ternyata ada kebijakan Kemenkeu yang membolehkan, atau datanya tidak
sesensitif dugaan ini, koreksi bagian ini — kesimpulan ini ditarik dari
dokumen, bukan dari keterangan langsung.
...
**Jangan ambil kutipan ini dari OpenSearch**, dua alasan:
...
2. Struktur index OpenSearch itu Dokumen → Blok (**satu blok per pasal**).
   Aturan teknik penyusunan ada di **Lampiran II** yang penomorannya butir,
   bukan pasal — pemotongan per-pasal kemungkinan besar tidak menghasilkan blok
   per butir. (Dugaan dari struktur yang terdokumentasi, belum diverifikasi ke
   isi index.)
...
## 11. Bentuk data Temuan

`docs/kontrak-data.md` di repo masih kosong padahal menurut berkas itu sendiri
ini prasyarat sebelum modul lain ditulis. Usulan awal:
...
**Ini usulan, belum keputusan** — perlu disetujui dulu sebelum `rules/` ditulis.
...
Fase 1 tidak butuh env di luar yang sudah ada di kerangka. Tabel ini untuk Fase
2/3, disiapkan sejak awal supaya `core/config.py` tidak dirombak ulang:
...
## 13. Peraturan untuk agen coding
...
## 15. Yang masih belum pasti

Jangan diperlakukan sebagai kebutuhan final:

1. **Daftar pemeriksaan Fase 1 belum dikonfirmasi ke penelaah harian.** Sebagian
   disusun dari logika dokumen dan ketentuan KMK 527, bukan dari keterangan
   orang yang mengerjakan telaah sehari-hari. Rencanakan sesi validasi sebelum
   menganggap Fase 1 selesai.
2. **Bentuk data Temuan (bagian 11) masih usulan.**
3. **Jalur pemasangan add-in** menunggu keterangan dari tim Law Analyzer.
4. **Lokasi database** (bagian 9) menunggu kepastian soal kerahasiaan rancangan.
5. **Nomor butir dan kutipan KMK 527** (bagian 10) belum diisi — menunggu
   pembacaan visual dokumen aslinya.
